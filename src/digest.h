 ...
 * game reads back -- WRAM, HRAM, OAM and both VRAM banks -- plus the sound
 * driver's WRAM section on its own are copied, the masked bytes zeroed, and
 * each CRC-32'd (zlib's polynomial, so Python's zlib.crc32 over the same
 * masked bytes is the reference side). One record per ordinal: five
 * little-endian uint32 in that order.
 *
 ...
 * oam, vram (0x4000, bank 0 then bank 1) and audio ($DD80-$DEE4 of WRAM). Blank lines and lines opening
