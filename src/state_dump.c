...
int runtime_write_state(const char *path, const RuntimeResult *runtime)
{
	...
	/* g_io is the backing store; the bus value a routine actually observes
	 * comes from gb_read8, which models joypad matrix resolution and the
	 * registers whose unused bits float high. Emulator references expose
