...
static void adapt_SaveDeckInDeckSaveMachine(ProbeState *s)
{
	SaveDeckInDeckSaveMachineResult result = SaveDeckInDeckSaveMachine();
	s->a = result.a;
	s->f = result.f;
}
/* <<< factory SaveDeckInDeckSaveMachine */
...
static void adapt_HandleAutoDeckMenu(ProbeState *s)
{
	HandleAutoDeckMenuResult r = HandleAutoDeckMenu();
...
static void adapt_GiftCenter_SendCard(ProbeState *s)
{
	GiftCenter_SendCardResult result = GiftCenter_SendCard();
	s->a = result.a;
	s->f = result.f;
}
/* <<< factory GiftCenter_SendCard */
...
static void adapt_HandleGiftCenter(ProbeState *s)
{
	HandleGiftCenter();
	(void)s;
}
...
const ProbeEntry probe_entries_deck_machine[] = {
	...
	{ "GiftCenter_SendCard", adapt_GiftCenter_SendCard },
	{ "GiftCenter_SendDeck", adapt_GiftCenter_SendDeck },
	{ "HandleGiftCenter", adapt_HandleGiftCenter },
	{ NULL, NULL },
};
