...
static void adapt_DeckNamingScreen_CheckButtonState(ProbeState *s)
{
	DeckNamingScreen_DrawCursorResult r = DeckNamingScreen_CheckButtonState();
	s->a = r.a;
	s->f = r.f;
	s->b = r.b;
	s->c = r.c;
...
static void adapt_InputPlayerName(ProbeState *s)
{
	FinalizeInputNameResult r = InputPlayerName(s->hl);
...
static void adapt_InputDeckName(ProbeState *s)
{
	FinalizeInputNameResult result =
		InputDeckName(s->a, s->b, s->c, s->d, s->e, s->hl);
	s->a = result.a;
	s->f = result.f;
	s->b = result.b;
	s->c = result.c;
...
const ProbeEntry probe_entries_input_name[] = {
	...
	{ "DeckNamingScreen_CheckButtonState", adapt_DeckNamingScreen_CheckButtonState },
	...
	{ "InputPlayerName", adapt_InputPlayerName },
	{ "InputDeckName", adapt_InputDeckName },
	{ NULL, NULL },
};
