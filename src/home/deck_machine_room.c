...
FuncD96cResult Func_d96c(uint8_t a)
{
	...
	uint8_t lo = entry[0]; uint8_t hi = entry[1];
	gb_write8(wTxRam2_ADDR, lo); gb_write8(wTxRam2_b_ADDR, lo);
...
void Script_BeatAaron(void)
{
	...
	gb_write8(event_addr, (uint8_t)((event & (uint8_t)~EVENT_AARON_BOOSTER_REWARD_MASK) | (value & EVENT_AARON_BOOSTER_REWARD_MASK)));
}
