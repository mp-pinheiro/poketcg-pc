...
/* duel/core.asm:4120-4126: FillRectangle's exit -- b 8, c 0, hl past the image. */
typedef struct { uint8_t b; uint8_t c; uint16_t hl; } ApplyCardCGBAttributesResult;
...
typedef struct { uint8_t f; } HandleDuelSetupResult;
...
typedef struct { uint8_t a; uint8_t c; uint8_t e; } PlayDeckShuffleAnimationResult;
...
/* duel/core.asm:6725-6731: everything but a is the effect command's exit
 * (TryExecuteEffectCommandFunction leaves its af in bc as well). */
typedef struct { uint8_t a; uint8_t f; uint8_t b; uint8_t c; uint8_t d; uint8_t e; uint16_t hl; } OppAction_ExecutePokemonPowerEffectResult;
...
typedef struct { uint8_t a, f, b, c, d, e; uint16_t hl; } OppActionBeginUseAttackResult;
...
typedef struct { uint8_t a; uint8_t f; } PlayAttackAnimation_DealAttackDamageSimpleResult;
...
typedef struct { uint8_t a; uint8_t f; uint8_t d; uint8_t e; } CheckIfCanDamageDefendingPokemonResult;
...
typedef struct { uint8_t a; uint8_t f; } CheckDamageToMrMimeResult;
...
typedef struct { uint8_t a; uint8_t f; uint8_t d; uint8_t e; } CheckIfAnyAttackKnocksOutDefendingCardResult;
...
typedef struct { uint8_t a; } OppActionUseAttackResult;
