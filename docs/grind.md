...
| `DIVERGE symbol=wAITrainerLogicCard writer=_AIProcessHandTrainerCards`, one byte, after an AI turn whose trainer decision returned no carry | `trainer_cards.asm:12-98`: the phase lives in `d`, the decide routine is called through `CallIndirect` with no `push de`, and a no-carry return goes to `.inc_hl_by_4` with the decide's exit `d` as the phase for the remaining rows of the same hand card (`AIDecide_EnergySearch` leaves the play-area count, so PHASE_12 finishes matching PHASE_04 rows and the byte ends as ITEM_FINDER; `AIDecide_ProfessorOak` leaves 0; `AIDecide_Defender_Phase13` preserves it). The C rescans with the real phase | every `AIDecide_*` takes `d` and reports its exit `d` (`TrainerDecision` carries it; the loop passes `phase` in), derived from the callees' `de` contracts, never a measured constant, and claimed in the contract's `compare` so the oracle checks it. 32 of 50 are derived and green (Bill, Imakuni, Gambler, ImposterProfessorOak, Recycle, Revive, Lass, ClefairyDoll, ItemFinder, Maintenance, Switch, Pokeball, PokemonFlute, MrFuji, EnergySearch, ProfessorOak, the PokemonTrader and ComputerSearch families) with `FindDuplicateCards`, `CheckIfCardCanBePlayed` and the four `common.c` deck/hand lookups widened too. The 18 that still rest on the damage estimators (`EstimateDamage_*`, `CheckIfSelectedAttackIsUnusable`, `AIDecideWhetherToRetreat`, `CheckIfCanDamageDefendingPokemon`, `FindBasicEnergyCardsInLocation` loops) do not claim `d`: FullHeal, PokemonCenter, Defender 13/14, PlusPower 13/14, Potion 07/10, SuperPotion 08/11, GustOfWind, EnergyRemoval, SuperEnergyRemoval, PokemonBreeder, EnergyRetrieval, SuperEnergyRetrieval, Pokedex, ScoopUp. The loop keeps `phase` until all 50 report: switching it with a placeholder regressed `grass-club` from 544666 to 132273, exactly where Defender_Phase13's preserved `d` was replaced by 0 (`grass-club` 544667, `psychic-club` 545891) |
| `DIVERGE field=hram symbol=hSCX writer=ApplyBackgroundScroll`, one byte off by one table step, `WINDOW reference_frames` > 1 | `scroll.asm:60-105` is scanline-raced: from the entry `rLY` it calls `GetNextBackgroundScroll` once per line up to `$60`, and the final `hSCX` is the next table step after that count. On a lag frame the VBlank handler enters mid-frame, so the count is the entry line, which no DoFrame-axis model carries; `scroll.c` always takes the `$60` iterations or the one-call path | not derivable from the asm alone: it needs the entry scanline of a lag frame. Do not widen the ledger and do not fit a constant; leave the p0 fact open (`science-club` 539292) and name it in the report |
