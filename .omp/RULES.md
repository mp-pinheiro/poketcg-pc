...
When, and only when, the user's trimmed message after slash-command expansion
is exactly `start`, treat it as the port-factory trigger. This is not a normal
porting request.
...
The project `/loop` command expands to its arguments, so `/loop start` is the
supported alias. Messages such as `/start`, `Start`, `start <issue>`, `/loop`,
`/loop start <issue>`, or prose containing `start` are ordinary requests and
do not activate this rule.
