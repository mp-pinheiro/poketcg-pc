...
from tests.cases._fixtures import (
    mason_preload_fixture as _mason_preload_fixture,
    MASON_PRELOAD_REGS as _MASON_PRELOAD_REGS,
)
...
def npc_pos(state_byte, x=0x55, y=0x66):
    return {
        EVENT_MASON_LAB_STATE_BYTE: bytes((state_byte,)),
        wLoadNPCXPos: bytes((x,)),
        wLoadNPCYPos: bytes((y,)),
    }
...
def sam_pos(state_byte, x=0x55, y=0x66, direction=0x77):
    return {
        ...
        wLoadNPCXPos: bytes((x,)),
        wLoadNPCYPos: bytes((y,)),
        wLoadNPCDirection: bytes((direction,)),
        wLoadedEventBits: b"\x00",
    }


SAM_READ = {
    EVENT_MASON_LAB_STATE_BYTE: 1,
    wLoadNPCXPos: 1,
    wLoadNPCYPos: 1,
    wLoadNPCDirection: 1,
    wLoadedEventBits: 1,
...
def tech5_pos(event_byte, x=0x55):
    return {
        EVENT_LEGENDARY_BYTE: bytes((event_byte,)),
        wLoadNPCXPos: bytes((x,)),
        wLoadedEventBits: b"\x00",
    }
...
CONTRACT["MasonLaboratoryAfterDuel"] = {
    "compare": ("a", "f", "b", "c", "d", "e", "hl"),
    "preserve": (),
    "wram_out": True,
}
...
def _pressed_a(legendary, x, y, direction):
    return {
        _LEGENDARY_BYTE: bytes((legendary,)),
...
CONTRACT["Func_d703"] = {"compare": ("a", "f"), "preserve": ("b", "c", "d", "e", "hl")}
CASES["Func_d703"] = [
    {"wram": {0xD3D8: b"\x00"}, "read": {0xD3D8: 1}},
    dict(POISON, wram={0xD3D8: b"\x00"}, read={0xD3D8: 1}),
    {
        **_mason_preload_fixture(vram=False, bank=1),
        "wram": {**_mason_preload_fixture(vram=False, bank=1)["wram"], 0xD3D8: b"\x02"},
        **_MASON_PRELOAD_REGS,
    },
]
...
for _i, _counts in enumerate(_TECH1_SEEDS):
    _base = (
        dict(POISON)
        if _i == 4
        else {"a": 0, "f": 0, "b": 0, "c": 0, "d": 0, "e": 0, "hl": 0}
    )
...
MUTATIONS["MasonLaboratoryAfterDuel"] = {
    "source_symbol": "MasonLaboratoryAfterDuel",
    "before": "	FindEndOfDuelScriptResult r = FindEndOfDuelScript(MasonLaboratoryAfterDuelTable);",
    "after": "	FindEndOfDuelScriptResult r = FindEndOfDuelScript((uint16_t)(MasonLaboratoryAfterDuelTable + 1u));",
    "case_ids": ["MasonLaboratoryAfterDuel-0"],
}
# <<< factory-mutation MasonLaboratoryAfterDuel
# >>> factory-mutation MasonLabCloseTextBox
MUTATIONS["MasonLabCloseTextBox"] = {
    "source_symbol": "MasonLabCloseTextBox",
    "before": "\tApplyOWMapEventChangeIfEventSet(MAP_EVENT_CHALLENGE_MACHINE);",
    "after": "\tApplyOWMapEventChangeIfEventSet((uint8_t)(MAP_EVENT_CHALLENGE_MACHINE + 1u));",
    "case_ids": ["MasonLabCloseTextBox-0", "MasonLabCloseTextBox-1"],
}
# <<< factory-mutation MasonLabCloseTextBox
# >>> factory-mutation Script_Tech1
MUTATIONS["Script_Tech1"] = {
    "source_symbol": "Script_Tech1",
    "before": "\tif (a >= 10u)",
    "after": "\tif (a > 10u)",
    "case_ids": ["Script_Tech1-0"],
}
# <<< factory-mutation Script_Tech1
MUTATIONS["Func_d703"] = {
    "source_symbol": "Func_d703",
    "before": "if (event == 0u)",
    "after": "if (event != 0u)",
