...
def adjusted_capacity(capacity: Capacity) -> tuple[Capacity, str | None]:
    ...
    if capacity.provider_throttled or (
        capacity.verifier_queue_p95 > capacity.verifier_soft_deadline
    ):
        target = max(4, capacity.job_slots // 2)
        return Capacity(
            job_slots=target,
...
def _dependency_ready(work: FactoryWork, by_issue: dict[int, FactoryWork]) -> bool:
    return all(
        dependency in by_issue
        and by_issue[dependency].state in {"done", "excluded"}
        for dependency in work.dependencies
    )


def _model_route(work: FactoryWork) -> str:
    if work.cohort or work.tier >= 2:
        return "task"
    return "smol"
...
def _assignment_kind(work: FactoryWork) -> str:
    if work.state == "recovery":
        return "task" if work.tier >= 2 or work.cohort else "completion"
    return "task" if work.cohort or work.tier >= 2 else "completion"
...
def _score(
    ...
    *,
    ...
    priority = _PRIORITY.get(work.priority, _PRIORITY["normal"])
    ...
    return priority, -leverage, -age, work.work_id
...
def _dependent_bytes(works: tuple[FactoryWork, ...]) -> dict[int, int]:
    ...
    for root in sizes:
        ...
        result[root] = sum(sizes[item] for item in seen)
    return result
...
def plan(
    ...
    dependency_bytes = _dependent_bytes(snapshot.works)
    fresh = [
    ...
    fresh.sort(key=lambda work: _score(work, dependent_bytes=dependency_bytes, now=now))
    recovery.sort(key=lambda work: _score(work, dependent_bytes=dependency_bytes, now=now))
    if snapshot.integration_ready and (
        not fresh and not recovery
        or len(snapshot.integration_ready) >= 8
    ):
        return FactoryPlan(
            snapshot_sha256=snapshot.sha256,
            integration=tuple(snapshot.integration_ready[:16]),
        )
    if available:
        selected: list[FactoryWork] = []
        recovery_slots = max(1, math.floor(capacity.job_slots / 5)) if recovery else 0
        queues = (
            [(recovery, recovery_slots), (fresh, available - recovery_slots)]
            if recovery_slots
            else [(fresh, available)]
        )
            ...
            return FactoryPlan(
                ...
                assignments=tuple(
                    PlannedAssignment(
                        ...
                        score=_score(work, dependent_bytes=dependency_bytes, now=now),
                    )
    ...
    if waits:
        return FactoryPlan(
            snapshot_sha256=snapshot.sha256,
            waiting_until=min(waits),
            waiting_reason="backoff",
        )
    paused = [work.issue_number for work in snapshot.works if work.state == "paused"]
    blocked = [
        work.issue_number for work in snapshot.works
        if work.state == "blocked" or not _dependency_ready(work, by_issue)
    ]
    if paused:
        return FactoryPlan(
            snapshot_sha256=snapshot.sha256,
            blocker_review=tuple(sorted(paused)),
        )
    if blocked:
        return FactoryPlan(
            snapshot_sha256=snapshot.sha256,
            dependency_analysis=tuple(sorted(blocked)),
        )
