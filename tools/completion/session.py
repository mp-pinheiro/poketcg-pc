...
def deck_seed(name: str, *, base: str, at: int, deck: int, cards: list[int], goal: str,
    ...
    if opponent_discard:
        ...
        taken: list[int] = []
        for discard_card in opponent_discard:
            if discard_card not in data:
                raise SessionError(f"{discard_card} is not a card")
            index = next((i for i, value in enumerate(opponent)
                          if value == ids[discard_card] and i not in taken), None)
            if index is None:
                raise SessionError(f"deck id {deck} does not hold {discard_card}")
            taken.append(index)
        pokes.setdefault(poke_at, []).extend(
            [(OPPONENT_DUEL_VARS + DUEL_BOARD["locations"] + index, CARD_LOCATION_DISCARD_PILE)
             for index in taken]
            + [(OPPONENT_DUEL_VARS + DUEL_BOARD["cards_in_discard"], len(taken))])
    if status is not None or stage is not None:
        if poke_at is None:
            raise SessionError("--poke-at names the ordinal the arena card is poked at")
        if status is not None:
            pokes.setdefault(poke_at, []).append(
                (PLAYER_DUEL_VARS + DUEL_BOARD["arena_status"], status))
        if stage is not None:
            pokes.setdefault(poke_at, []).append(
                (PLAYER_DUEL_VARS + DUEL_BOARD["arena_stage"], stage))
    masks = base_masks[:at]
    then = then or []
    if then:
        import explore
...
def board_seed(name: str, *, card: str, attack: int | None, base: str, at: int, deck: int,
    ...
    bench_entries: list[tuple[str, int | None, int]] = []
    for item in bench or []:
        parts = item.split(":")
        piece = parts[0]
        hp_text = parts[1] if len(parts) > 1 else ""
        energy_text = parts[2] if len(parts) > 2 else ""
        bench_entries.append((piece, int(hp_text) if hp_text else None,
                              int(energy_text) if energy_text else 0))
    if len(bench_entries) > BENCH_SLOTS:
        raise SessionError(f"{len(bench_entries)} bench cards, {BENCH_SLOTS} slots")
    bench_indices = place([piece for piece, _hp, _energy in bench_entries], taken)
    bench_energy: list[list[int]] = []
    energy_name = wanted[0] if wanted else "DOUBLE_COLORLESS_ENERGY"
    for _piece, _hp, wanted_energy in bench_entries:
        bench_energy.append(place([energy_name] * wanted_energy, taken))
    ...
    for slot, attached in enumerate(bench_energy):
        writes += [(PLAYER_DUEL_VARS + DUEL_BOARD["locations"] + index,
                    CARD_LOCATION_BENCH_1 + slot) for index in attached]
    for slot, (index, (piece, hp, _energy)) in enumerate(zip(bench_indices, bench_entries, strict=True)):
    ...
    writes += [(PLAYER_DUEL_VARS + DUEL_BOARD["arena_card"], arena_index),
               (PLAYER_DUEL_VARS + DUEL_BOARD["arena_hp"],
                data[card]["hp"] if arena_hp is None else arena_hp),
               (PLAYER_DUEL_VARS + DUEL_BOARD["arena_stage"], 0),
               (PLAYER_DUEL_VARS + DUEL_BOARD["arena_flags"],
                0 if arena_flags is None else arena_flags),
               ...
               (PLAYER_DUEL_VARS + DUEL_BOARD["not_in_deck"],
                ...
                + len(bench_indices) + sum(len(a) for a in bench_energy))]
    if hand_indices:
...
def main(argv: list[str] | None = None) -> int:
    ...
    seed_parser.add_argument("--stage", type=int, default=None,
                             help="DUELVARS_ARENA_CARD_STAGE value for the player's arena card")
